...
export default function GPSCheckMissionPage() {
  ...
  const validate = () => {
    ...
    if (failedDestination === "return-to-friends" && !isGroupMission && !friends.some((friend) => friend.user_id === friendRecipient)) nextErrors.push("ไม่มีเพื่อนสำหรับส่งเงิน")
  ...
  const createMission = async () => {
    ...
    const mission: Mission = {
      ...
      friendRecipient: failedDestination === "return-to-friends" && !isGroupMission ? friendRecipient : undefined,
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-[minmax(0,1.35fr)_minmax(320px,0.65fr)]">
          ...
          <div className="space-y-6">
            ...
            <section className="rounded-[28px] border border-white/70 bg-white/75 p-6 shadow-[0_20px_60px_rgba(18,18,18,0.08)] backdrop-blur-xl">
              ...
              <div className="mt-3 space-y-3">{[["return-to-friends", isGroupMission ? "หารให้เพื่อนในกลุ่ม" : "คืนให้เพื่อน"], ["donate-to-foundation", "บริจาคมูลนิธิ"], ["support", "สนับสนุน go."]].map(([value, label]) => <div key={value} className={`rounded-xl border-2 bg-[#f9f9f8] p-3 transition ${failedDestination === value ? "border-[#AFFF00] bg-[#AFFF00]/10" : "border-[#121212]/10 hover:border-[#AFFF00]/40"}`}>
            ...
            <section className="rounded-[28px] border border-[#AFFF00]/50 bg-[#f4f9e9] p-6"><h2 className="text-xl font-bold">7. GPS Verification</h2><p className="mt-3 text-sm text-gray-700">You must be within {isGroupMission ? 20 : 10} meters of the destination at the required time.</p><div className="mt-4 grid grid-cols-2 gap-3 text-sm"><div><div className="text-xs text-gray-500">Required Distance</div><strong>{isGroupMission ? 20 : 10} meters</strong></div><div><div className="text-xs text-gray-500">Required Time</div><strong>{startDateTime.toLocaleTimeString("th-TH", { hour: "2-digit", minute: "2-digit" })}</strong></div></div></section>
