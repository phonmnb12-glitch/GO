...
- Solo > GPS Check: you pick a destination on a map when creating the mission, then must physically arrive within 10 meters of it before the mission's end time. Location is checked automatically in the background while the mission page is open.
- Group > Work Team: you invite friends to a shared assignment. Nobody is charged until EVERY invited friend accepts -- then everyone's pledge is held at once. Any member can submit a photo of their finished work; AI scores how well it matches the assignment (0-100%), and every OTHER member must confirm it before that person's own pledge is released. This mirrors Photo AI's checkpoint idea but for group work with peer confirmation instead of just AI checkpoints.
- Group > GPS Check: same idea as Solo GPS Check, but for an invited group -- everyone's pledge is held together once all invites are accepted.
