-- 1. Reject a Work Team mission whose start time is already in the past.
-- 2. Let the creator assign each invited friend their own task/subject
--    (e.g. one friend does the intro, another does the conclusion) --
--    stored per mission_members row, graded individually against that
--    member's own submission instead of just the shared mission
--    description.
alter table public.mission_members add column if not exists assigned_task text;

-- Adding a new parameter changes the function's signature, which Postgres
-- treats as a distinct overload rather than a replacement -- drop the old
-- 8-argument version first so only one create_work_team_mission exists and
-- PostgREST can never resolve an RPC call to the stale one.
drop function if exists public.create_work_team_mission(text, text, text, timestamptz, timestamptz, numeric, uuid[], text);
...
grant execute on function public.create_work_team_mission(text, text, text, timestamptz, timestamptz, numeric, uuid[], text, jsonb) to authenticated;
